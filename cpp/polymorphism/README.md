...
Most of **pbyte.h** is relatively straightforward.
One gnarly aspect is the need to use an inner class to allow [] to index bits
...
* **variable_token** concrete class holding the name of a variable
...
that returns a unique pointer to a copy of the token. Subclasses override
...
1. Tokenizing: turning the input into a set of tokens
...
To find the correct operator token whose symbol matches a given word, we use one of the
...
```
An object that behaves like a function, specifically a unary-function that
...
We can pass this "function" to the `find_if` algorithm, it tests it for each item in the sequence.

### Lambda functions

The above example shows the use of anonymous or lambda functions supplied as
arguments to the `binary_op_token` constructor. Lambda functions are specified like
```
[](pbyte l, pbyte r) { return l & r;}
```
The square brackets indicate a *capture* where local variables of the surrounding function can be passed in.
Then there are the two function arguments and the function method body.
...
others. First, we test if the sequence starts with a variable and an assignment
```
    if(tokens.size()>2) {
...
If the test passes, then we can `dynamic_cast` the tokens to the appropriate type
```
    variable_token& vt = dynamic_cast<variable_token&>(e0);
    assign_op_token& ot = dynamic_cast<assign_op_token&>(e1);
```
Now we want to evaluate the remainder of the sequence. Here we can use a bit of
...
For example, if there are three tokens, the first being a variable
the second a binary operator and the third a number, we get the
values corresponding to the variable and number use the operators
`eval` method.
```
    pbyte l, r;
    variable_token& vt = dynamic_cast<variable_token&>(e0);
...
and a third to unenroll a student.
